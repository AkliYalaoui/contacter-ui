...
const Profile = () => {
  ...
  return (
    <div className="my-10 relative">
      <header className="shadow-2xl border border-gray-100 rounded">
        <img
          alt="profile"
      ...
      <section className=" flex flex-wrap items-center justify-center">
        ...
        <div className="text-gray-600 p-3 text-center">
          <h4 className="text-gray-800 font-semibold">About</h4>
          ...
          <div className="flex flex-wrap space-x-1 items-center justify-center px-2 py-6">
            {userProfile.friends.map((friend) => (
              <Link key={friend._id} to={`/profile/${friend._id}`}>
                <div className="rounded border w-32  border-gray-300 shadow-lg text-center">
                  <img
                    alt="profile"
                    className="w-32 h-32 object-cover m-auto"
                    src={`http://localhost:8080/api/users/image/${friend.profilePhoto}`}
                  />
                  <h3 className="p-2 text-gray-600">{friend.userName}</h3>
                </div>
              </Link>
            ))}
          </div>
